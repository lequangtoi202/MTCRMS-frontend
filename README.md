...
- Set `NEXT_PUBLIC_API_BASE_URL` to the backend origin, for example `http://localhost:3000`
- Set `NEXT_PUBLIC_API_PREFIX` if the backend prefix differs from `/api/v1`
- Set `NEXT_PUBLIC_APP_NAME` if the deployed app name differs from `MTCRMS`

When building a Docker image, pass `NEXT_PUBLIC_API_BASE_URL` and optionally `NEXT_PUBLIC_API_PREFIX` as build args because `NEXT_PUBLIC_*` values are compiled into the frontend bundle.
